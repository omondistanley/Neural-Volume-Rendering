...
def sample_images_at_xy(
    ...
    # Ensure xy_grid is on the same device and dtype as images to avoid grid_sample device errors
    xy_grid = xy_grid.to(device=images.device, dtype=images.dtype)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    sample = torch.randperm(xy_grid.shape[0])[:n_pixels]
    xy_grid_sub = xy_grid[sample]
