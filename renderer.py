...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        # computing opacity for segment i -- from image in pdf
        alpha_val = 1 - torch.exp(-rays_density * deltas)
        transmittance = torch.cumprod(
        torch.cat([torch.ones_like(alpha_val[:, :1, :]), (1.0 - alpha_val + eps)], dim=1),
        dim=1
    )[:, :-1, :]
        # TODO (1.5): Compute weight used for rendering from transmittance and alpha
        weights = transmittance * alpha_val
