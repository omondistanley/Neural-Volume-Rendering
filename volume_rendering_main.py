...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            plt.imsave('images/grid.png', grid_vis)
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            # Get sample points from the ray bundle, then render
            sample_points = ray_bundle.sample_points  # Shape: (n_rays, n_samples, 3)
            points_flat = sample_points.view(-1, 3)
            render_points('images/sample_points.png', points_flat.unsqueeze(0), image_size[0])
        ...
        # TODO (Q1.5): Visualize color
        if cam_idx == 0 and file_prefix == '':
            color_vis = out['feature'].view(image_size[1], image_size[0], 3).detach().cpu().numpy()
            color_vis = np.clip(color_vis, 0, 1)  # Ensure values are in [0,1] range
            plt.imsave('images/color.png', color_vis)
...
def train(
    ...
    center_init = np.array(model.implicit_fn.sdf.center.data.detach().cpu()).tolist()[0]
    side_lengths_init1 = np.array(model.implicit_fn.sdf.side_lengths.data.detach().cpu()).tolist()[0]
    center_init_rounded1 = tuple([round(c, 2) for c in center_init])
    side_lengths_init_rounded1 = tuple([round(s, 2) for s in side_lengths_init1])
    print(f"Box center->before training: {center_init_rounded1}")
    print(f"Box side lengths->before training: {side_lengths_init_rounded1}")
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            # Predicted colors for the sampled pixels (one RGB per ray)
    ...
    # Extract the raw lists of coordinates/lengths
    center_raw = np.array(model.implicit_fn.sdf.center.data.detach().cpu()).tolist()[0]
    side_lengths_raw = np.array(model.implicit_fn.sdf.side_lengths.data.detach().cpu()).tolist()[0]

    # Use a list comprehension to round each element to 2 decimal places
    center_rounded = tuple([round(c, 2) for c in center_raw])
    side_lengths_rounded = tuple([round(s, 2) for s in side_lengths_raw])


    #   Print the rounded tuples
    print(f"Box center->before training: {center_init_rounded1}")
    print(f"Box side lengths->before training: {side_lengths_init_rounded1}")


    print("Box center:", center_rounded)
    print("Box side lengths:", side_lengths_rounded)
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            # Ensure shapes are compatible: rgb_gt and rgb_pred should be (N,3)
